...
# CPU package energy via RAPL and GPU energy via NVML, both collected by powerlog.
...
#
# Output: results/cpugpu/{tc,sg}/<Dataset>_<Engine>.csv  (resumable)
set -u
...
MNMG=$(pwd)
PROF="powerlog --output"      # powerlog measures CPU+GPU by default
GDLOG=$HOME/gdlog
BJOIN=$HOME/batch_joins
mkdir -p results/cpugpu/tc results/cpugpu/sg

have() { [ -s "$1" ] && grep -q "GPU Energy" "$1"; }   # resumable
...
profile_gdlog(){ local o=$1 app=$2 f=$3 m=${4:-}; have "$o" && { echo "skip $o"; return; }; ( cd "$GDLOG" && powerlog --output "$o" "./build/$app" "$f" $m ); }
BJOIN_MEM=${BJOIN_MEM:-90}   # % free GPU mem for BJoin; lower (e.g. 80) if async-alloc aborts on the 40GB PCIE card
profile_bjoin(){ local o=$1 app=$2 f=$3; have "$o" && { echo "skip $o"; return; }; ( cd "$BJOIN/build" && powerlog --output "$o" "./$app" "$f" "$BJOIN_MEM" ); }
profile_cudf() { local o=$1 prog=$2 f=$3; have "$o" && { echo "skip $o"; return; }; $PROF "$o" "$PY" "related/cudf_programs/$prog" "$f"; }

echo "############ TC ############"
for d in $TC_SETS; do
  echo ">>> TC $d"
  profile_mpi   "$MNMG/results/cpugpu/tc/${d}_MNMGDatalog.csv" ./tc.out    "data/${BIN[$d]}.bin"
  profile_mpi   "$MNMG/results/cpugpu/tc/${d}_INLJoin.csv"     ./tc_nl.out "data/${BIN[$d]}.bin"
  profile_gdlog "$MNMG/results/cpugpu/tc/${d}_GPULog.csv"      TC "./data/${GDIR[$d]}/edge.facts" 0
  profile_bjoin "$MNMG/results/cpugpu/tc/${d}_BJoin.csv"       TC "../data/${BTXT[$d]}.txt"
done
for d in $CUDF_TC; do
  profile_cudf "$MNMG/results/cpugpu/tc/${d}_cuDF.csv" tc.py "data/${TXT[$d]}.txt"
done

echo "############ SG ############"
for d in $SG_SETS; do
  echo ">>> SG $d"
  profile_mpi   "$MNMG/results/cpugpu/sg/${d}_MNMGDatalog.csv" ./sg.out    "data/${BIN[$d]}.bin"
  profile_mpi   "$MNMG/results/cpugpu/sg/${d}_INLJoin.csv"     ./sg_nl.out "data/${BIN[$d]}.bin"
  profile_gdlog "$MNMG/results/cpugpu/sg/${d}_GPULog.csv"      SG "./data/${GDIR[$d]}/edge.facts"
  profile_bjoin "$MNMG/results/cpugpu/sg/${d}_BJoin.csv"       SG "../data/${BTXT[$d]}.txt"
done
for d in $CUDF_SG; do
  profile_cudf "$MNMG/results/cpugpu/sg/${d}_cuDF.csv" sg.py "data/${TXT[$d]}.txt"
done

echo "############ DONE ############"
echo "Combine: python ../analysis/cpu_gpu_energy_tables.py results/cpugpu/tc"
echo "         python ../analysis/cpu_gpu_energy_tables.py results/cpugpu/sg"
